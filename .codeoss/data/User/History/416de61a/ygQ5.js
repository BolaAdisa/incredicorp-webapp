  ...
  const handleTransfer = async (e) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "users", currentUserId), {
        balance: currentUserBalance - transferAmount, // Deduct as a number
      });
  
      const recipientBalance = Number(recipientUser.balance) || 0; // Ensure recipient balance is a number
      await updateDoc(doc(db, "users", recipientUserId), {
        balance: recipientBalance + transferAmount, // Add as a number
      });

      await addDoc(collection(db, "transactions"), {
        date: new Date().toISOString().split('T')[0], // Get current date in YYYY-MM-DD format
        amount: transferAmount, // Use the transferAmount (number)
        sentTo: recipientEmail, 
        userId: currentUserId,
        type: "transfer", // You can add a type to distinguish transfers from other transactions
      });
